...
public final class MusicAPIService: @unchecked Sendable, ObservableObject {
    ...
    deinit {
        ...
        timeoutTimer?.invalidate()  // 新增

    }

    private func startBackgroundCookieRefresh() {
        backgroundTimer?.invalidate()
        backgroundTimer = Timer.scheduledTimer(withTimeInterval: autoRefreshInterval, repeats: true)
        { [weak self] _ in
            self?.triggerBackgroundCookieRefresh()
        }

        timeoutTimer = Timer.scheduledTimer(withTimeInterval: 5.0, repeats: true) {
            [weak self] _ in
    ...
    public func validateCookieIfNeeded() {
        guard Date().timeIntervalSince(lastValidationTime) > 30 else {
            print("[MusicAPI] Skipping validation, too recent")
            return
        }
        ...
        Task {
            do {
                ...
                if let httpResponse = response as? HTTPURLResponse {
                    print(
                        "[MusicAPI] Heartbeat: received response, status=\(httpResponse.statusCode)"
                    )
    ...
    public func searchSongs(keyword: String, page: Int = 1, pageSize: Int = 30) async throws
        -> [Song]
    {
        ...
        return try await fetchHiCNSearch(
            keyword: keyword, page: page, pageSize: pageSize, platform: currentPlatform)
    }
    ...
    public func getSongURL(
        songId: String, format: AudioFormat, sign: String? = nil, time: Int? = nil
    ) async throws -> String {
        let cachedSign = sign ?? signCache[songId] ?? ""
        let cachedTime = time ?? timeCache[songId]
        return try await fetchHiCNSongURL(
            songId: songId, format: format, platform: currentPlatform, sign: cachedSign,
            time: cachedTime)
    }

    // MARK: - Get Lyrics

    public func getLyrics(songId: String, sign: String? = nil, time: Int? = nil) async throws
        -> String
    {
    ...
    private func fetchHiCNSearch(keyword: String, page: Int, pageSize: Int, platform: MusicPlatform)
        async throws -> [Song]
    {
        ...
        let body =
            "platform=\(platform.rawValue)&keyword=\(encodedKeyword)&page=\(page)&size=\(pageSize)"
        request.httpBody = body.data(using: .utf8)
    ...
    private func fetchHiCNSongURL(
        songId: String, format: AudioFormat, platform: MusicPlatform, sign: String = "",
        time: Int? = nil
    ) async throws -> String {
        ...
        var body =
            "platform=\(platform.rawValue)&songid=\(songId)&format=\(format.rawValue)&bitrate=\(bitrate)&time=\(requestTime)"
        if !sign.isEmpty {
            body += "&sign=\(sign)"
        }
        ...
        if let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any],
           let code = json["code"] as? Int, code != 0 {
            let msg = json["msg"] as? String ?? "unknown"
            print("[MusicAPI] Search API error: code=\(code), msg=\(msg)")
            // 搜索接口异常时清掉 sign 缓存，下次重搜会拿新的
            signCache.removeAll()
            timeCache.removeAll()
            throw MusicAPIError.serverError
        }
